...
const blockPropsSchemas = {
  ...
  doctorList: z.object({ specialtyFilter: z.number().int().positive().optional(), showSearch: z.boolean().optional(), limit: z.number().int().positive().max(100).optional() }).strip(),
  specialtyGrid: z.object({ columns: z.union([z.literal(3), z.literal(4), z.literal(6)]), showCount: z.number().int().positive().max(100).optional() }).strip(),
  serviceGrid: z.object({ columns: columns2to4, showCount: z.number().int().positive().max(100).optional() }).strip(),
  studyGrid: z.object({ columns: columns2to4, showCount: z.number().int().positive().max(100).optional() }).strip(),
  newsGrid: z.object({ limit: z.number().int().positive().max(50), columns: columns2to4 }).strip(),
  mapEmbed: z.object({ embedHtml: html, height: z.number().int().min(160).max(900).optional() }).strip(),
  ...
  cta: z.object({ title: z.string().trim().min(1).max(180), text: z.string().max(500).optional().or(z.literal("")), ctaLabel: z.string().trim().min(1).max(80), ctaHref: z.string().trim().min(1).max(500), background: z.string().max(80).optional().or(z.literal("")) }).strip(),
  stats: z.object({ items: z.array(z.object({ value: z.string().trim().min(1).max(40), label: z.string().trim().min(1).max(120) }).strip()).max(12) }).strip(),
  logos: z.object({
    heading: z.string().max(180).optional().or(z.literal("")),
